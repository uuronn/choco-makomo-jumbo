...
export function RoomsClient({ initialToken }: RoomsClientProps) {
  ...
  const { data: havingCharacters } = useUserCharacterList(user?.uid ?? null, initialToken)

  // APIリクエストのトークン処理を修正
  const createRoom = async () => {
    if (!user) return
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/rooms/create`, {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        hostUserId: user.uid,
        characterIdList: selectedCharacters.map((character) => character.characterId),
      }),
    })
  ...
  const joinRoom = async () => {
    if (!user) return
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/rooms/join`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${initialToken}`,
  ...
  const startCpuBattle = async () => {
    if (!user) return
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/cpu-battle/create`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${initialToken}`,
  ...
  useEffect(() => {
    const fetchRooms = async () => {
      const roomsRes = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/rooms`, {
        headers: {
          Authorization: `Bearer ${initialToken}`,
        },
      })
  ...
  const handleSelectCharacter = (character: Character) => {
    if (selectedCharacters.find((c) => c.characterId === character.characterId)) {
      setSelectedCharacters(selectedCharacters.filter((c) => c.characterId !== character.characterId))
          ...
          <div className="bg-gray-900/80 border border-green-400/30 rounded-lg p-4 h-full">
            ...
            <div className="flex flex-col gap-4">
              {selectedCharacters.length > 0 ? (
                selectedCharacters.map((character) => (
                  ...
                  >
                    <div className="relative w-12 h-12 overflow-hidden rounded-lg">
                      <Image
                        src={characterToImagePath(character.characterId) || "/placeholder.svg" || "/placeholder.svg"}
                        alt={character.name}
                        fill
                    ...
                    <div className="flex-1">
                      <div className="font-medium text-green-200">{character.name}</div>
                      <div className="text-xs text-emerald-400">Lv.{character.level}</div>
                    </div>
                    <Button
                      variant="ghost"
                      size="icon"
                ...
                <div className="text-green-400/50 text-center w-full py-4">技術を選択してください（最大3体）</div>
              )}
            </div>
          </div>
        </section>
                ...
                {havingCharacters?.map((character) => (
                  ...
                  >
                    <div className="flex flex-col items-center">
                      ...
                      <div className="text-center font-medium text-green-200 truncate w-full">{character.name}</div>
                      <div className="text-center text-xs text-emerald-400">Lv.{character.level}</div>
                    </div>
                    <Button
                      variant="ghost"
                      size="icon"
        ...
        <div className="mb-3 border border-green-400/30 rounded-lg p-3 backdrop-blur-sm backdrop-filter bg-black/20">
          ...
          <div className="mt-2 flex items-center gap-4">
            <div className="flex items-center gap-3 bg-green-400/10 rounded-lg p-2 flex-1">
              ...
              <div>
                <p className="text-green-200 text-sm font-medium">{user?.displayName || "あなた"}</p>
                <p className="text-xs text-green-400/70">選択中: {selectedCharacters.length}体</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 bg-green-400/10 rounded-lg p-2 flex-1">
              ...
              <div>
                <p className="text-green-200 text-sm font-medium">{"selectedRoom.host_user.name"}</p>
                <p className="text-xs text-green-400/70">選択中: {"selectedRoom.character_list?.length" || 0}体</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {showCpuOptions ? (
          ...
          <>
            <h2 className="text-lg font-bold mb-2 text-green-400 flex items-center">
              ...
              <Button onClick={refreshRooms} className="ml-2 bg-green-400 text-black hover:bg-green-500 text-sm h-9">
                更新
              </Button>
            </h2>
            ...
            <div className="space-y-3 h-[calc(100%-40px)]">
              ...
              <div className="overflow-x-auto h-[calc(100%-40px)]">
                <div className="flex h-full gap-4 items-center">
                  {rooms
                    .filter((room) => room.guest_user === null && room.host_user.id !== user?.uid)
                    .map((room) => (
                      <div
                        key={room.id}
                        className={cn(
                      ...
                      >
                        ...
                        <div className="p-2 text-center">
                          <h4 className="font-bold text-green-400 text-sm">{room.host_user.name}</h4>
                        </div>
                      </div>
                    ))}
                </div>
