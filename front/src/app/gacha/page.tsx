...
export default function TabbedGacha() {
	...
	const pullGacha = async () => {
		...
		// API call with timeout to simulate server request
		setTimeout(async () => {
	...
	const handleTabChange = (value: string) => {
		if (isAnimating) return; // Don't allow tab change during animation
		setActiveTab(value);
		resetAnimations();
		setShowResult(false);
		setResult(null);
		...
		>
			...
			<div className="py-3 gap-0 w-full h-full max-w-md bg-black/80 backdrop-blur-sm rounded-xl shadow-[0_0_15px_rgba(0,255,128,0.3)] border border-green-500/30 overflow-hidden relative z-10">
				...
				<div className="bg-gradient-to-r from-green-900/80 to-green-700/80 p-4 text-center relative">
					{/* <div className="absolutee top-0 left-0 w-full h-1 bg-gradient-to-r from-green-400 via-green-300 to-green-400"></div> */}
					...
					>
						...
						<div className="p-2 flex flex-col items-center relative">
							...
							{activeTab === "github" && !isAnimating && !showResult && (
								<div className="w-full mb-6 absolute">
									{/* <div className="text-center mb-4">
										<h2 className="text-xl font-bold text-green-300 mb-2">
											GitHubリポジトリからキャラクターを取得
										</h2>
										<p className="text-sm text-green-200/70">
											GitHubリポジトリのURLを入力して、対応するキャラクターを取得します。
										</p>
									</div> */}
									...
									<div className="relative">
										...
										<Input
											...
											className="pl-10 bg-black/50 border-green-500/30 text-green-300 placeholder:text-green-500/50 focus:border-green-400 focus:ring-green-400/30"
										/>
									</div>
									...
									{/* <div className="mt-4 text-xs text-green-500/70 font-mono text-center">
										<p>
											対応しているリポジトリからキャラクターを一体獲得できます
										</p>
										<p className="mt-1">
							...
							<div className="relative w-64 h-64 mb-6 flex items-center justify-center">
								{/* Energy field effect during animation */}
											...
											>
												...
												<div className="w-32 h-32 rounded-full bg-black border border-green-500/50 flex items-center justify-center relative overflow-hidden">
													...
													{isAnimating ? (
														...
														>
															{activeTab === "github" ? (
																<Github className="h-12 w-12 text-green-400 drop-shadow-[0_0_8px_rgba(0,255,128,0.8)]" />
															) : (
														...
														>
															{activeTab === "github" ? (
																<Github className="h-12 w-12 text-green-500" />
															) : (
							...
							<div className="w-full flex items-center gap-3 justify-center">
								...
								{activeTab === "regular" ? (
									...
									<Button
										onClick={getCharacterByGithubUrl}
										disabled={isAnimating || !githubUrl.trim()}
									...
									>
										...
										<div className="relative flex items-center justify-center gap-2">
											...
											<span className="tracking-wider font-mono">取得する</span>
											<motion.div
												className="absolute inset-0 bg-green-400/20 mix-blend-overlay"
												animate={{ top: ["100%", "-100%"] }}
												transition={{
