...
export default function GithubGacha() {
	...
	const [confettiCanvasRef, setConfettiCanvasRef] =
	...
	const triggerConfetti = () => {
		try {
			if (confettiCanvasRef && typeof confetti !== "undefined") {
				const myConfetti = confetti.create(confettiCanvasRef, {
					resize: true,
					useWorker: true,
				});
	...
	const getCharacterByGithubUrl = async () => {
		...
		try {
			...
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_BASE_URL}/api/gacha/github`,
				{
					method: "POST",
					headers: {
	...
	return (
		...
		>
			{/* 紙吹雪用のキャンバス */}
			<canvas
				ref={(ref) => setConfettiCanvasRef(ref)}
					...
					{!isAnimating && !showResult && (
						<div className="w-full mb-6">
							...
							<div className="mt-4 text-xs text-green-500/70 font-mono text-center">
								<p>対応しているリポジトリからキャラクターを一体獲得できます</p>
								<p className="mt-1">例: https://github.com/facebook/react</p>
							</div>
						</div>
					)}
